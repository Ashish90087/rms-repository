...
save_users(data: any){
 ...
 save_stocks(data: any){
  ...
  save_dummy(data: any){
    console.log(data)
      return this.http.post('http://localhost:3000/users/dummy',data).pipe(tap(res=>{res}),
      catchError(e=>{
        throw new Error(e);
      }));

    }
    save_dummy2(data: any, id1: any){
      console.log(data);
      console.log("ID exist in dummy 2",id1);
      console.log("DATA exist in dummy 2",data);
        return this.http.put('http://localhost:3000/users/dummy/update',data).pipe(tap(res=>{res}),
        catchError(e=>{
          throw new Error(e);
        }));
  ...
  updateDummy(data : any){
    return this.http.get('http://localhost:3000/users/dummy',data).pipe(tap(res=>{res}),
    catchError(e=>{
      throw new Error(e);
    }));
  }
